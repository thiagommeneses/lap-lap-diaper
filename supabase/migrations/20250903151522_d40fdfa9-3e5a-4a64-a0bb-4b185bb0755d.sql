-- Fix the get_user_detailed_stats function - GROUP BY issue with dag.created_at
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'age_groups', (
      SELECT json_agg(
        json_build_object(
          'id', age_groups_data.id,
          'name', age_groups_data.name,
          'age_range', age_groups_data.age_range,
          'estimated_quantity', age_groups_data.estimated_quantity,
          'price_per_unit', age_groups_data.price_per_unit,
          'color_theme', age_groups_data.color_theme,
          'icon_name', age_groups_data.icon_name,
          'current_stock', age_groups_data.current_stock,
          'created_at', age_groups_data.created_at
        ) ORDER BY age_groups_data.created_at
      )
      FROM (
        SELECT 
          dag.id,
