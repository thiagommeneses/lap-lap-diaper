-- CORREÇÃO DE SEGURANÇA CRÍTICA: Remover TODOS os dados legados expostos publicamente

-- 1. Remover registros relacionados que referenciam age groups com user_id NULL
DELETE FROM public.diaper_purchases 
WHERE age_group_id IN (
  SELECT id FROM public.diaper_age_groups WHERE user_id IS NULL
);

DELETE FROM public.diaper_donations 
WHERE age_group_id IN (
  SELECT id FROM public.diaper_age_groups WHERE user_id IS NULL
);

-- 2. Agora remover os dados legados com user_id NULL (dados sensíveis antigos)
...
-- 3. Atualizar RLS policies para REMOVER acesso público (sem user_id IS NULL)
...
-- 4. Garantir que user_id seja obrigatório (NOT NULL) para prevenir futuros vazamentos
