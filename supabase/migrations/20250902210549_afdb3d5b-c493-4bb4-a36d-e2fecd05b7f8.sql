...
-- Atualizar RLS policies para diaper_usage para ser por usuário
...
-- Adicionar user_id para diaper_usage também
...
-- Criar view pública para últimos doadores (sem informações pessoais)
CREATE OR REPLACE VIEW public.recent_donors AS
SELECT 
  donor_name,
  donation_date,
  quantity,
  diaper_age_groups.name as age_group_name
FROM public.diaper_donations
JOIN public.diaper_age_groups ON diaper_donations.age_group_id = diaper_age_groups.id
